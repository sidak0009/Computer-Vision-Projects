...
class poseDetector:
    ...
    def findPose(self, img, draw=True):
        landmark_drawing_spec = self.mpDraw.DrawingSpec(color=(0, 0, 255), thickness=2,
                                                        circle_radius=2)  # Red color for landmarks
        connection_drawing_spec = self.mpDraw.DrawingSpec(color=(0, 255, 0), thickness=2)  # Green color for connections
        imgRgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        self.results = self.pose.process(imgRgb)
        if self.results.pose_landmarks:
            if draw:
                self.mpDraw.draw_landmarks(img, self.results.pose_landmarks,
                                           self.mpPose.POSE_CONNECTIONS, landmark_drawing_spec,
...
def main():
    ...
    while True:
        ...
        cv2.putText(img, (str(int(fps))),
                    (70, 50), cv2.FONT_HERSHEY_PLAIN, 3, (0, 255, 0), 3)
